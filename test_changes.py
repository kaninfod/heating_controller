...
def test_deep_copy_schedule_generation():
    ...
    stay_home_week = generator.generate_stay_home_schedule(
        base_week, swap_day="tuesday"
    )
...
def test_env_variables():
    ...
    removed = [
        "thermostat_entities",
        "temperature_sensor_entities",
        "humidity_sensor_entities",
    ]
...
def test_mode_manager_thermostat_mapping():
    ...
    if re.match(r"^[a-z0-9\s\(\)]+$", z2m_device_name):
        print(
            f"✓ PASS: Z2M device name is valid for MQTT: zigbee2mqtt/{z2m_device_name}/set"
        )
    else:
