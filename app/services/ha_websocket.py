...
from app.models.state import ThermostatState, SensorState, ConnectionStatus, SystemState
...
class HomeAssistantWebSocket:
    ...
    def __init__(self, url: str, access_token: str, monitored_entities: List[str]):
        ...
        # Response queue for RPC calls (message_id -> response)
        self.pending_responses: Dict[int, asyncio.Queue] = {}
    ...
    async def _fetch_initial_states(self):
        ...
        await self.websocket.send(json.dumps({"id": msg_id, "type": "get_states"}))
    ...
    async def _handle_message(self, data: Dict[str, Any]):
        ...
        # Route responses to pending RPC calls
        if msg_type == "result" and msg_id in self.pending_responses:
            queue = self.pending_responses[msg_id]
    ...
    def _update_entity_state(self, state: Dict[str, Any]):
        ...
        if entity_id.startswith("climate."):
            ...
            try:
                sensor_value = (
                    float(state_value) if state_value != "unavailable" else None
                )
    ...
    async def fetch_areas(self) -> List[Dict[str, Any]]:
        ...
        Returns list of area objects with: area_id, name, icon, etc.
        """
        msg_id = self._get_message_id()
        ...
        try:
            ...
            # Send request
            await self.websocket.send(
                json.dumps({"id": msg_id, "type": "config/area_registry/list"})
            )
            ...
            if data.get("success"):
                ...
                logger.error(
                    f"Failed to fetch areas: {data.get('error', {}).get('message', 'Unknown error')}"
                )
                return []
        except asyncio.TimeoutError:
            logger.error("Timeout fetching areas from Home Assistant")
            return []
        except Exception as e:
    ...
    async def fetch_entities(self) -> List[Dict[str, Any]]:
        """
        Fetch all entities from Home Assistant entity registry
        Returns list of entity objects with: entity_id, domain, area_id, device_class, etc.
        """
        msg_id = self._get_message_id()
        ...
        try:
            ...
            if data.get("success"):
                ...
                logger.error(
                    f"Failed to fetch entities: {data.get('error', {}).get('message', 'Unknown error')}"
                )
                return []
        except asyncio.TimeoutError:
            logger.error("Timeout fetching entities from Home Assistant")
            return []
        except Exception as e:
    ...
    async def fetch_sensor_entities(self) -> List[Dict[str, Any]]:
        """
        Fetch all sensor entities with device_class from Home Assistant
        Returns list of state objects for sensors with device_class in ['temperature', 'humidity']
        Each sensor includes: entity_id, state, attributes (with device_class), device_id
        """
        msg_id = self._get_message_id()
        ...
        try:
            ...
            if data.get("success"):
                ...
                # Filter to sensors with temperature or humidity device_class
                sensor_states = [
                    s
                    for s in states
                    if s.get("entity_id", "").startswith("sensor.")
                    and s.get("attributes", {}).get("device_class")
                    in ["temperature", "humidity"]
                ]

                logger.info(
                    f"Fetched {len(sensor_states)} temperature/humidity sensors from Home Assistant"
                )
                return sensor_states
            else:
                logger.error(
                    f"Failed to fetch sensor entities: {data.get('error', {}).get('message', 'Unknown error')}"
                )
                return []
        except asyncio.TimeoutError:
            logger.error("Timeout fetching sensor entities from Home Assistant")
            return []
        except Exception as e:
            logger.error(f"Error fetching sensor entities: {e}")
            return []
        finally:
            # Clean up queue
            if msg_id in self.pending_responses:
    ...
    async def fetch_devices(self) -> List[Dict[str, Any]]:
        """
        Fetch all devices from Home Assistant device registry
        Returns list of device objects with: id, area_id, name, manufacturer, etc.
        """
        msg_id = self._get_message_id()
        ...
        try:
            ...
            if data.get("success"):
                ...
                logger.info(f"Fetched {len(devices)} devices from Home Assistant")
                return devices
            else:
                logger.error(
                    f"Failed to fetch devices: {data.get('error', {}).get('message', 'Unknown error')}"
                )
                return []
        except asyncio.TimeoutError:
            logger.error("Timeout fetching devices from Home Assistant")
            return []
        except Exception as e:
