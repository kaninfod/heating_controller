...
class ScheduleGenerator:
            ...
            with open(self.day_types_path, "r") as f:
                ...
                if "day_types" in data:
                    ...
                    if isinstance(day_types_data, dict):
                        self.day_types = {
                            dt_id: dt_data["schedule"]
                            for dt_id, dt_data in day_types_data.items()
                        }
                    else:
                        self.day_types = {
                            dt["id"]: dt["schedule"] for dt in day_types_data
                        }
                else:
                    # Direct format: {"workday": {"schedule": "..."}, ...}
                    self.day_types = {
                        dt_id: dt_data["schedule"] for dt_id, dt_data in data.items()
                    }
            logger.info(
                f"Loaded {len(self.day_types)} day types: {list(self.day_types.keys())}"
            )
    ...
    def get_day_schedule(self, day_type_id: str) -> str:
        ...
        if day_type_id not in self.day_types:
            raise ValueError(
                f"Day type '{day_type_id}' not found. Available: {list(self.day_types.keys())}"
            )
        ...
        if len(pairs) != 6:
            logger.warning(
                f"Day type '{day_type_id}' has {len(pairs)} pairs, expected 6"
            )
    ...
    def generate_week_schedule(
        self, week_composition: Dict[str, str]
    ) -> Dict[str, str]:
        """
    ...
    def generate_stay_home_schedule(
        self, base_week: Dict[str, str], swap_day: str = None
    ) -> Dict[str, str]:
        """
        ...
        logger.info(
            f"Generating stay-home schedule, swapping {swap_day} to weekend_day"
        )
