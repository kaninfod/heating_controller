"""
Schedule API routes
Manage heating schedules and apply them to thermostats/zones
"""
...
from app.dependencies import HAClient, ZoneManagerDep, ScheduleManagerDep
...
async def get_schedules(schedule_manager: ScheduleManagerDep):
    ...
    for schedule in schedules:
        ...
        expanded_schedule = schedule_manager.generator.generate_week_schedule(week_composition)
...
async def create_schedule(schedule_data: ScheduleCreate, schedule_manager: ScheduleManagerDep):
...
async def update_schedule(schedule_id: str, schedule_data: ScheduleUpdate, schedule_manager: ScheduleManagerDep):
    ...
    try:
        ...
        if "week" in update_dict:
            test_schedule = {**schedule_manager.get_schedule(schedule_id).model_dump(), **update_dict}
...
async def apply_schedule(
    ...
    """
    Apply a schedule to a thermostat or zone
    
    Provide either thermostat_id or zone_id
    """
    
    if not request.thermostat_id and not request.zone_id:
        raise HTTPException(
            status_code=400,
            detail="Must provide either thermostat_id or zone_id"
        )
    
    if request.thermostat_id and request.zone_id:
        raise HTTPException(
            status_code=400,
            detail="Cannot provide both thermostat_id and zone_id"
        )
    
    try:
        if request.thermostat_id:
            # Apply to single thermostat
            success = await schedule_manager.apply_schedule_to_thermostat(
                ha_client,
                request.thermostat_id,
                schedule_id
            )
            ...
            # Apply to zone
            results = await schedule_manager.apply_schedule_to_zone(
                ha_client,
                zone_manager,
                request.zone_id,
                schedule_id
            ...
            if success_count == total_count:
                return {
                    ...
                    "thermostats": list(results.keys()),
                    "message": f"Applied schedule {schedule_id} to all {total_count} thermostats in zone"
                }
            else:
                return {
                    "success": False,
                    ...
                    "zone_id": request.zone_id,
                    "results": results,
                    "message": f"Applied to {success_count}/{total_count} thermostats"
                }
    
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Error applying schedule: {str(e)}"
        )
