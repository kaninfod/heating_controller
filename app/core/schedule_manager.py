...
class ScheduleManager:
    ...
    def update_schedule(self, schedule_id: str, schedule_data: dict) -> Schedule:
        ...
        # Update schedule
        schedule = Schedule(**{**self.schedules[schedule_id].model_dump(), **schedule_data})
    ...
    async def apply_schedule_to_thermostat(
        ...
        try:
            ...
            if not mode_success:
                logger.warning(f"Failed to set {thermostat_id} to auto mode, continuing anyway...")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(f"Retry {attempt + 1}/{max_retries} for {thermostat_id} in {wait_time}s")
            ...
            logger.error(f"Failed to apply schedule {schedule_id} to {thermostat_id} after {max_retries} attempts")
            return False
    ...
    async def apply_schedule_to_zone(
        self,
        ha_client,
        zone_manager,
        zone_id: str,
        ...
        """
        Apply a schedule to all thermostats in a zone
        
        Returns dict of {thermostat_id: success}
        """
        results = {}
        
        # Get thermostats in the zone
        thermostat_ids = zone_manager.get_zone_thermostats(zone_id)
        
        if not thermostat_ids:
            logger.warning(f"No thermostats in zone {zone_id}")
            return results
        ...
        # Update zone's active schedule
        if all(results.values()):
            zone_manager.assign_schedule_to_zone(zone_id, schedule_id)
            logger.info(f"Applied schedule {schedule_id} to all thermostats in zone {zone_id}")
        else:
            logger.warning(f"Some thermostats failed when applying schedule to zone {zone_id}")
